...
if transformar is False:
  ...
  datos.sort_values(myvar0, inplace=True)
  myvar = f"{myvar0}_trans"
```

## Transformacion de datos

Una opcion es la normalizacion/estandarizacion de la variable a una con media de 0 y desviación estándar de 1.

```{python}
if transformar:
  tns, tv = stats.probplot(datos[myvar0], fit=False)
  var_z = tns
```

```{python}
# if transformar:
#   var_z, tv, tns = geostats.nscore(datos, myvar0)
```
...
if transformar:
  ...
  ax.axvline(x=bc_lambda, color='r', linestyle='--')
  plt.show()
...
ax = outline_sf.plot(facecolor='none', edgecolor='cyan', linewidth=1.25)
datos_sf.plot(column=myvar, cmap='Spectral', legend=True, 
              legend_kwds={'label': myvar, 'shrink': 0.8}, ax=ax)
ax.set_xlabel(x_map)
ax.set_ylabel(y_map)
plt.show()
```
...
variog_omni = (p9.ggplot(dat_vgm, p9.aes(x = 'lag', y = 'gamma')) + 
  # p9.geom_point(p9.aes(size = 'npair')) +
  p9.geom_point(size = 3, color='blue') + 
...
variog_dir = (p9.ggplot(dat_vgm2,p9.aes(x = 'lag', y = 'gamma',
...
stb_model = gs.Stable(dim=2, var=C1, len_scale=a, nugget=C0, alpha=1.5, rescale=3)  # 0 < alpha < 2
sph_model = gs.Spherical(dim=2, var=C1, len_scale=a, nugget=C0)
circ_model = gs.Circular(dim=2, var=C1, len_scale=a, nugget=C0)
mat_model = gs.Matern(dim=2, var=C1, len_scale=a, nugget=C0, nu=0.5, rescale=3) # 0.2 < nu < 30
...
mat_model.plot(ax=ax, label = 'Matern')
...
```{python}
lags_fit = np.linspace(0, bin_center.max(), 100)
gamma_fit = fit_model.variogram(lags_fit)
df_fit = pd.DataFrame({'lag': lags_fit, 'gamma': gamma_fit})

# (variog_omni + 
#   p9.geom_line(p9.aes(x='lag', y='gamma'), data=df_fit, color='black', size=1.25, linetype='dashed', inherit_aes=False))
...
A continuacion se presentan tres funciones para realizar la validacion cruzada: Leave-One-Out Cross-Validation (LOOCV), k-fold Cross-Validation (k-fold CV) y Spatial Cross-Validation (SCV). Para cuando se tienen pocos datos se recomienda usar LOOCV, mientras que para conjuntos de datos mas grandes se puede usar k-fold CV con k=5 o k=10.

```{python}
#| label: loocv
"""
LOOCV con gstools (Kriging).
Asume:
...
def kfold_gstools(fit_model, coords, values, k=5, normalizer=None, shuffle=True, random_state=None):
...
#| label: spatial-kfold
"""
Validación cruzada espacial k-fold para kriging usando gstools.

- Métodos para generar los grupos espaciales:
    * "kmeans": agrupa las coordenadas en k grupos espaciales (por defecto)
    * "block": bloques de una rejilla regular — especificar block_size
- Usa GroupKFold para mantener grupos espaciales completos fuera de cada grupo.
- Devuelve predicciones por punto, varianzas predictivas y métricas comunes.

Entradas:
 - fit_model: instancia de modelo de covarianza de gstools (ya configurada, no necesariamente ajustada)
 - coords: array-like con forma (n,2) de x,y
 - values: array-like con forma (n,) de observaciones
 - n_splits: número de grupos espaciales
 - method: "kmeans" o "block"
 - block_size: tamaño de celda (requerido para method="block")
 - normalizer: objeto normalizador de gstools o None
 - random_state: semilla para reproducibilidad

La función retorna un diccionario con preds, pred_vars, residuals, metrics y etiquetas de grupo.
"""
...
def make_spatial_groups(coords, k=5, method="kmeans", block_size=None, random_state=None):
    coords = np.asarray(coords)
    if method == "kmeans":
...
def spatial_kfold_gstools(
    ...
    metrics = {
        ...
        "r": stats.pearsonr(field,results['preds'])[0],
...
```{python}
#| label: spatial-kfold-results

results_spatialk = spatial_kfold_gstools(
  fit_model, 
  coords, 
  field, 
  k=5, 
...
```{python}
results = results_kfold  # results_loo, results_kfold, o results_spatialk
```

```{python}
#| label: xval-plot-map

datos_sf['fold'] = results['groups']
datos_sf['fold'] = datos_sf['fold'].astype('category')

ax = outline_sf.plot(facecolor='none', edgecolor='cyan', linewidth=1.25)
datos_sf.plot(column='fold', cmap='Spectral', legend=True,
              legend_kwds={'title': 'Fold', 'bbox_to_anchor': (1.02, 1), 'loc': 'upper left'},
              ax=ax
              )
fig = ax.get_figure()
...
print(f"MSDR: {results['metrics']['msdr']:<.4f}")
print(f"r: {results['metrics']['r']:<.4f}")
print(f"R^2: {results['metrics']['r2']:<.4f}")
print(f"Bias: {results['metrics']['bias']:<.4f}")
```
...
```{python}
krig_pred, krig_var = krig.structured([x_grid, y_grid])
```

```{python}
if transformar and transformacion == "Quantile":
  # pred_var = nst_trans.inverse_transform(krig_pred.reshape(-1,1)).reshape(krig_pred.shape)
  # var_var = nst_trans.inverse_transform(krig_var.reshape(-1,1)).reshape(krig_var.shape)
  pred_var = np.interp(krig_pred.ravel(),tns,tv).reshape(krig_pred.shape)
  var_var = np.interp(krig_var.ravel(),tns,tv).reshape(krig_var.shape)
  # var_var = krig_var
  std_var = np.sqrt(var_var)
else:
  pred_var = krig_pred
  var_var = krig_var
  # var_var = krig_var if transformar is False else bc_norm.denormalize(krig_var)
  std_var = np.sqrt(var_var)
```
...
# Predicción
pcm0 = axes[0].pcolormesh(grid_x, grid_y, pred_var.T, cmap='viridis')
...
# Desviación estándar
pcm1 = axes[1].pcolormesh(grid_x, grid_y, std_var.T, cmap='plasma')
...
fig.suptitle('Kriging - Predicción y Desviación estándar')
plt.tight_layout()
plt.show()
```
...
# im = ax.imshow(pred_var.T, extent=extent, origin='lower', cmap='viridis', aspect='equal')
...
da = xr.DataArray(
    ...
    attrs={
        "long_name": f"Kriging Prediccion"
    }
)
print(da) # DataArray

# da.rio.write_crs(datos_sf.crs, inplace=True) # asignar CRS si se conoce ej "EPSG:5367"
...
pred_arr = np.flipud(pred_var.T)
var_arr = np.flipud(var_var.T)
std_arr = np.flipud(std_var.T)

ds = xr.Dataset(
    {
        "pred_var": (("y", "x"), pred_arr),
        "var_var":  (("y", "x"), var_arr),
        "std_var":  (("y", "x"), std_arr),
    },
...
fig = go.Figure(data=[go.Surface(z=pred_var.T, x=grid_x, y=grid_y, colorscale="Viridis")])

# Agregar etiquetas y título
fig.update_layout(
    title="Mapa 3D de Predicción",
    scene=dict(xaxis_title=x_map, 
               yaxis_title=y_map, 
               zaxis_title=myvar0, 
               aspectmode = 'manual', 

...
Se transforma (normaliza/estandariza) la variable a una con media de 0 y desviación estándar de 1, esto se logra utilizando la transformacion de cuantiles (`QuantileTransformer`).

```{python}
#| eval: false
#| include: false

if transformar: datos[myvar], tv, tns = geostats.nscore(datos, myvar0)
```
...
outline_sf = gpd.GeoDataFrame(geometry=[outline], crs=None)
```
...
# da.rio.write_crs(None, inplace=True) # asignar CRS si se conoce ej "EPSG:4326"
...
print(f"Rasterguardado en: {archivo}")
```

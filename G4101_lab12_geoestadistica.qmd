...
datos_sf.plot(column=myvar, cmap='Spectral_r', legend=True, 
...
- `bin_edges` las distancias (lags) a las cuales calcular el variograma
...
df_vmap = pd.DataFrame(
    {"x": XX.ravel(), "y": YY.ravel(), "gamma": vmap.ravel(), "npair": npmap.ravel()}
)
...
```{python}
markers = {0: "o", 45: "^", 90: "s", 135: "D"}
palette = {0: "#4DAF4A", 45: "#FF7F00", 90: "#9E9AC8", 135: "#F781BF"}

fig_dir, ax_dir = plt.subplots(figsize=(7, 5))
...
ax_dir.axhline(1.0, color="red", linestyle="--", linewidth=1)
ax_dir.set_xlabel("Distancia [m]")
ax_dir.set_ylabel("Semivarianza")
ax_dir.set_ylim(bottom=0)

# handles, labels = ax_dir.get_legend_handles_labels()
# ax_dir.legend(handles, labels, title="Direccion", loc="upper left", frameon=False,
#           ncol=4, bbox_to_anchor=(0, 1.05))
...
```{python}
x_smooth = np.linspace(0, dat_vgm2["lag"].max(), 200)
gamma_model = fit_model.variogram(x_smooth)
...
results_loo = cv_gs.gs_loocv(
  fit_model, 
  coords, 
  field, 
  normalizer=bc_norm if transformar and transformacion=="Box-Cox" else None,
  transformar=transformar,
  transformacion=transformacion if transformar else None
  )
```
...
results_spatialk = cv_gs.gs_spatial_kfold(
  fit_model, 
  coords, 
  field, 
  k=5, 
...
```{python}
results = results_spatialk  # results_loo, results_kfold, o results_spatialk
```

```{python}
#| label: xval-plot-map

datos_sf['fold'] = results['kcv_df']['fold']
...
im = ax.imshow(
    pred_var.T, extent=extent, origin="lower", cmap="viridis", aspect="equal"
)
outline_sf.boundary.plot(ax=ax, color="white", linewidth=1.25)
...
fig = go.Figure(
    data=[go.Surface(z=pred_var.T, x=grid_x, y=grid_y, colorscale="Viridis")]
)

...
stats.f.ppf(1-a,len(theta)-1,len(theta2)-1)
```
...
```{python}
pg.circular.circ_r(thetarad) # resultante media
stats.vonmises.fit(thetarad)[0] # parametro concentracion
stats.circstd(thetarad) # desviacion estandar
stats.circvar(thetarad) # varianza
```

```{python}
pycirc.descriptive.circ_r(thetac.alpha) # resultante media
pycirc.descriptive.circ_std(thetac.alpha) # desviacion estandar
pycirc.descriptive.circ_var(thetac.alpha) # varianza
pycirc.descriptive.circ_kappa(thetac.r, thetac.n) # parametro concentracion
```
...
```{python}
pycirc.hypothesis.watson_test(thetac.alpha, verbose=True)
pycirc.hypothesis.rao_spacing_test(thetac.alpha, verbose=True)
pycirc.hypothesis.kuiper_test(thetac.alpha, verbose=True)
```
...
```{python}
pycirc.hypothesis.V_test(angle=np.radians(200), alpha=thetac.alpha, verbose=True)
```
